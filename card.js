...
var unfoldTitle = unfold.querySelector(".bottom_text");
var unfoldArrow = unfold.querySelector(".action_arrow");
...
unfoldTitle.addEventListener("click", toggleAdditionalData);
unfoldArrow.addEventListener("click", toggleAdditionalData);

var params = new URLSearchParams(window.location.search);

function loadReadyData(result) {
  if (!result || !result["name"] || !result["surname"] || !result["year"] || !result["month"] || !result["day"]) {
    return;
  }
...
function setImage(image) {
  try {
    var userImage = null;
    try { userImage = localStorage.getItem('moby_uploaded_image'); } catch(e) { userImage = null; }
    // if the user uploaded an image, prefer it and don't overwrite
    if (userImage) {
      return;
    }
